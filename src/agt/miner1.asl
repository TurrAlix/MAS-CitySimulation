...
@pih2[atomic]
+!init_handle(Gold)
  :  pos(X,Y)                       // There's gold at position X,Y!!
...
+!handle(gold(X,Y))
  ...
  <- .print("Handling ",gold(X,Y)," now.");
     !pos(X,Y);                 // go to the gold position
     !ensure(pick, gold(X,Y));  // pick up the gold
     ?depot(_,DX,DY);
     !pos(DX,DY);                 // go to the depot
     !ensure(drop, 0);          // drop the gold
     .print("Finish handling ", gold(X,Y));
      ?score(S);                
     -+score(S+1);                      // update score + 1
     .send(leader,tell,dropped);        // .send(Receiver, Type, Content)
     .print("My score is: ", S+1);
     !!choose_gold.                     // look for another gold
...
+!ensure(drop, _) : carrying_gold & pos(DX,DY) & depot(_,DX,DY)
  <- drop.
...
/*
+winning[source(leader)] : score(A, S) & my_name(A) 
    <- .print("I won with score ", S).
*/

+winning(A, S)[source(leader)] : .my_name(A)
   <-  -winning(A,S);       // per dire che l'ha gestito, lo toglie
